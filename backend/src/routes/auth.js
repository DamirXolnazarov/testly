...
router.post("/login", loginRateLimiter, async (req, res) => {
  ...
  try {
    ...
    res.json({ token, admin: { email: admin.email, fullName: admin.fullName, mustChangePassword: !!admin.mustChangePassword, isSuperadmin: !!admin.isSuperadmin } });
  } catch (e) {
...
router.patch("/me", requireAdmin, async (req, res) => {
  ...
  try {
    ...
    if (Object.keys(patch).length === 0) {
      ...
      const token = signToken(admin);
      return res.json({ token, admin: { email: admin.email, fullName: admin.fullName, mustChangePassword: !!admin.mustChangePassword, isSuperadmin: !!admin.isSuperadmin } });
    }
    ...
    res.json({ token, admin: { email: updated.email, fullName: updated.fullName, mustChangePassword: !!updated.mustChangePassword, isSuperadmin: !!updated.isSuperadmin } });
  } catch (e) {
